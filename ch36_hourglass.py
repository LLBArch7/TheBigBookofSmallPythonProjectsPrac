...
# (!) Try changin this to any number between 0 and 100:
WIDE_FALL_CHANCE = 50
...
def runHourglassSimulation(allSand):
    ...
    while True: # Keep looping until sand has run out.
        random.shuffle(allSand) # Random order of grain simulaiton.
        ...
        # If no sand has move on this step, reset the hourglass:
        if not sandMovedOnThisStep:
            ...
            for sand in allSand:
                bext.goto(sand[X], sand[Y])
                print(' ', end='')
            break # Break out of main simulaiton loop.
