...
def getWords():
    """Return a list of 12 words that could possible be the password.
...
def getComputerMemoryString(words):
    ...
    # Pick on line per word to contain a word. There are 16 lines, but
    # they are spit into two halves.
    ...
    for lineNum in range(16): # The "computer memory" has 16 ines.
...
def askForPlayerGuess(words, tries):
    ...
    while True:
        print(f'Enter password: ({tries} remaining)')
        guess = input('> ').upper()
        if guess in words:
            return guess
