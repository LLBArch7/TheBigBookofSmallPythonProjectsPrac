...
def main():
    print(f'''Hungry Robots, by Al Sweigart al@inventwithpython.com
          ...
          without being caught. You have a personal teleporter device, but it onlu
...
def getNewBoard():
    """Returns a distionary that represents the board. The keys are
    (x, y) tuples of integer indexes for board positions, the values are
    WALL, EMPTY, SPACE, or DEAD_ROBOT. The dictionary also has the key
...
def askForPlayerMove(board, robots, playerPosition):
    ...
    while True:
        ...
        if move == 'QUIT':
            ...
            # Retun the new player position based on their move:
...
def moveRobots(board, robotPositions, playerPosition):
    ...
    while len(robotPositions) > 0:
        ...
        # Check if it moved into a robot, then destroy both robots:
