...
# (!) Try changing the contants to create a fish tank with only kelp,
# or only bubblers.
...
# The x and y positions where a fish runs int o the edge of the screen:
LEFT_EDGE = 0
...
def simulateAquarium():
    ...
    for fish in FISHES:
        ...
        if STEP % fish['vSpeed'] == 0:
            if fish['goingDown']:
                ...
                if fish['y'] != TOP_EDGE:
                    fish['y'] -= 1 # Move teh fish up.
                else:
                    fish['goingDown'] = True # Turn the fish around.
                    
        # Fish can randomly change their verticle direction:
    ...
    for i in range(len(BUBBLES) - 1, -1, -1):
        if BUBBLES[i]['y'] == TOP_EDGE: # Delete bubble that reach the top.
            del BUBBLES[i]
