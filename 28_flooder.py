...
def main():
    ...
    bext.fg('white')
    bext.clear()
    print('''Flooder, by All Sweigart al@inventwithpython.co
          
          Set the upper left color/shap, which fills in all the
          adjacent squres of that color/shape. Try to make the
...
def displayBoard(board, displayMode):
    ...
    for y in range(BOARD_HEIGHT):
        ...
        if y == 0: # The first row begins with '>'.
            print('>', end='')
        else: # Later rows begin with a white verticle line.
            print(UPDOWN, end='')
            
        # Display ech tile in this row:
        for x in range(BOARD_WIDTH):
